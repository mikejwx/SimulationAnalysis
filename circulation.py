...
ax.set_title('Rectangle and cloud lwp')
plt.savefig('../area_of_interest.png', dpi =150)
plt.show()
...
ax0.plot(H_control, V_sb_sim_list[0], 'b*', markersize = 15, label = 'V$_{sb,sim}$, control/max')
ax0.plot(H_exp01, V_sb_sim_list[1], 'r*', markersize = 15, label = 'V$_{sb,sim}$, exp01/max')
ax0.plot(H_exp02, V_sb_sim_list[2], 'g*', markersize = 15, label = 'V$_{sb,sim}$, exp02/max')
ax0.plot(H_sim_full_control, [V_sb_sim['control'][i] for i in i_times10], 'bo', label = 'V$_{sb,sim}, control$')
ax0.plot(H_sim_full_exp01[::2], [V_sb_sim['exp01'][i] for i in i_times20], 'ro', label = 'V$_{sb,sim}, exp01$')
ax0.plot(H_sim_full_exp02[::2], [V_sb_sim['exp02'][i] for i in i_times20], 'go', label = 'V$_{sb,sim}, exp02$')
...
ax0.set_ylim([0, 6])
ax0.text(75, 0.25, '$V_{sb}(KF) = 0.78(\\frac{g H l_x}{\\rho c_p T_0 U_{sc}})^{0.5}$', fontsize = 16)
plt.legend(loc = 2, ncol = 2)

ax1 = fig.add_subplot(1, 2, 2)
ax1.plot(H_prediction, W_sb_prediction(H_prediction), 'k--', label = 'W$_{sb}$ (KF15)')
ax1.plot(H_control, W_sb_sim_list[0], 'b*', markersize = 15, label = 'W$_{sb,sim}$, control/max')
ax1.plot(H_exp01, W_sb_sim_list[1], 'r*', markersize = 15, label = 'W$_{sb,sim}$, exp01/max')
ax1.plot(H_exp02, W_sb_sim_list[2], 'g*', markersize = 15, label = 'W$_{sb,sim}$, exp02/max')
ax1.plot(H_sim_full_control, [W_sb_sim['control'][i] for i in i_times10], 'bo', label = 'W$_{sb,sim}, control$')
ax1.plot(H_sim_full_exp01[::2], [W_sb_sim['exp01'][i] for i in i_times20], 'ro', label = 'W$_{sb,sim}, exp01$')
ax1.plot(H_sim_full_exp02[::2], [W_sb_sim['exp02'][i] for i in i_times20], 'go', label = 'W$_{sb,sim}, exp02$')
ax1.set_title('Maximum Vertical Velocity')
ax1.set_xlabel('Sensible Heat Flux (W m$^{-2}$)')
ax1.set_ylabel('Vertical Velocity (m s$^{-1}$)')
ax1.set_ylim([0, 5])
ax1.text(75, 0.25, '$W_{sb}(KF) = 1.25(\\frac{z_{cb}}{l_y}\\frac{g H l_x}{\\rho c_p T_0 U_{sc}})^{0.5}$', fontsize = 16)
plt.legend(loc = 2, ncol = 2)

plt.savefig('../KF15_comparison.png', dpi = 150)
plt.show()
