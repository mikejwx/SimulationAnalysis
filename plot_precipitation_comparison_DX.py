...
fig = plt.figure(figsize = (6, 10))
for key in keys:
    idx = [it for it in range(len(data_dict[key]['lwp_times'])) if (360.0 >= data_dict[key]['lwp_times'][it])]
...
cbar_ax = fig.add_axes([0.12, 0.1, 0.78, 0.02])
cbar = fig.colorbar(im, cax = cbar_ax, orientation = 'horizontal', label = 'Morning Cloud Frequency')
cbar.ax.set_xticklabels([str(level) for level in my_cldfreq_levels])
plt.subplots_adjust(bottom = 0.17, wspace = 0.17, hspace = 0.30)
plt.savefig('../DX_comparison_morning_cloud_frequency.png', dpi = 150, bbox_inches = 'tight')
plt.show()
