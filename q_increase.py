...
for day in days:
    ...
    if day == '01':
        ...
        t_len = q_inc.variables[q_lsrain_key].shape[0]*ndays
    ...
    rain[days.index(day)*144:(days.index(day)+1)*144, :] = np.mean(q_inc.variables[q_lsrain_key][:], axis = (2,3))
    
    bl_cld[days.index(day)*144:(days.index(day)+1)*144, :] = np.mean(q_inc.variables[q_blpcld_key][:], axis = (2,3))
    
    advection[days.index(day)*144:(days.index(day)+1)*144, :] = np.mean(q_inc.variables[q_advect_key][:], axis = (2,3))
    
    diffusion[days.index(day)*144:(days.index(day)+1)*144, :] = np.mean(q_inc.variables[q_diffus_key][:], axis = (2,3))
    
    qt_bal_cld[days.index(day)*144:(days.index(day)+1)*144, :] = np.mean(q_inc.variables[q_qtbcld_key][:], axis = (2,3))
    
    total[days.index(day)*144:(days.index(day)+1)*144, :] = np.mean(q_inc.variables[q_totali_key][:], axis = (2,3))
    
    idealised[days.index(day)*144:(days.index(day)+1)*144, :] = np.mean(q_inc.variables[q_ideali_key][:], axis = (2,3))
    
    zi[days.index(day)*144:(days.index(day)+1)*144] = np.mean(zi_day.variables[zi_key][:], axis = (1,2))
    ...
    zf[days.index(day)*144:(days.index(day)+1)*144] = np.mean(freezing_level, axis = (1,2))
    ...
    zt[days.index(day)*144:(days.index(day)+1)*144] = np.mean(tropopause_level, axis = (1,2))
...
def my_plot_style(variable, my_title, file_name, show = True):
    ...
    fig = plt.figure()
    ax = plt.subplot()
    CL = ax.contourf(times/1440., z, np.transpose(variable), levels = np.linspace(np.min(variable), np.max(variable), 21), cmap = 'bwr', vmin = -np.max(np.abs(variable)), vmax = np.max(np.abs(variable)))
    ax.plot(times/1440., zi, color = 'k', lw = 2, label = 'Boundary Layer Depth')
    ax.plot(times/1440., zf, color = 'b', lw = 2, ls = '--', label = 'Freezing Level')
    ax.plot(times/1440., zt, color = 'r', lw = 2, ls = ':', label = 'Tropopause Level')
    ax.legend(loc = 2)
