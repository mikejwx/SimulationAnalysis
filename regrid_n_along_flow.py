...
def createSwathNC(hour):
    ...
    input_data = in_plane_winds(input_nc.variables[u_key][:]*1., input_nc.variables[v_key][:]*1., orientation = orientation_in)[1]
    ...
    output_var = np.zeros((input_nc.variables[u_key].shape[0], input_nc.variables[u_key].shape[1], swath_x.shape[0], swath_x.shape[1]))
    for it in xrange(input_nc.variables[u_key][:].shape[0]):
    ...
    time_dim = output_nc.createDimension(time_key, input_nc.variables[u_key][:].shape[0])
    z_dim    = output_nc.createDimension('thlev_zsea_theta', input_nc.variables[u_key][:].shape[1])
    ...
    out_var   = output_nc.createVariable(var_out, np.float64, (time_key, 'thlev_zsea_theta', 'y_prime', 'x_prime'))
...
send_email(message = 'Completed n swath for ' + my_path.split('/')[-2] + ' experiment.', subject = 'regrid_n_along_flow.py', attachments = [''], isAttach = False)
