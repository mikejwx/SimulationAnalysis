...
resolutions = ['1600m', '1600mS']

paths = {'1600mS' : '/nerc/n02/n02/xb899100/CloudTrail/Control_1600m_HRIC_INV_S/',
         '1600m'  : '/nerc/n02/n02/xb899100/CloudTrail/Control_1600m_HRIC_INV/'}
...
### Make the plot boxplot timeseries ###
fig = plt.figure(figsize = (10, 6), tight_layout = True)
res = resolutions[0]
axa = fig.add_subplot(2, 2, 1)
# Plot the median
axa.plot(my_data[res][ctz_time_key], [np.nanpercentile(ob, 50) for ob in my_data[res]['max_ctz']], 'k', lw = 2)
# Plot the interquartile range
axa.fill_between(my_data[res][ctz_time_key], [np.nanpercentile(ob, 25) for ob in my_data[res]['max_ctz']], [np.nanpercentile(ob, 75) for ob in my_data[res]['max_ctz']], facecolor = 'k', edgecolor = 'None', alpha = 0.5)
# Plot the 90th percentile
axa.plot(my_data[res][ctz_time_key], [np.nanpercentile(ob, 90) for ob in my_data[res]['max_ctz']], 'k', ls = '--')
# Plot the 10th percentile
axa.plot(my_data[res][ctz_time_key], [np.nanpercentile(ob, 10) for ob in my_data[res]['max_ctz']], 'k', ls = '--')
# Plot the maximum
axa.plot(my_data[res][ctz_time_key], [np.nanmax(ob) if len(ob) > 0 else np.nan for ob in my_data[res]['max_ctz']], 'rx')
# Panel formatting
...
axa.set_title('a) DX' + res[:-1])

res = resolutions[1]
axb = fig.add_subplot(2, 2, 2)
# Plot the median
axb.plot(my_data[res][conv_time_key], [np.nanpercentile(ob, 50) for ob in my_data[res]['max_ctz']], 'k', lw = 2)
# Plot the interquartile range
axb.fill_between(my_data[res][conv_time_key], [np.nanpercentile(ob, 25) for ob in my_data[res]['max_ctz']], [np.nanpercentile(ob, 75) for ob in my_data[res]['max_ctz']], facecolor = 'k', edgecolor = 'None', alpha = 0.5)
# Plot the 90th percentile
axb.plot(my_data[res][conv_time_key], [np.nanpercentile(ob, 90) for ob in my_data[res]['max_ctz']], 'k', ls = '--')
# Plot the 10th percentile
axb.plot(my_data[res][conv_time_key], [np.nanpercentile(ob, 10) for ob in my_data[res]['max_ctz']], 'k', ls = '--')
# Plot the maximum
axb.plot(my_data[res][conv_time_key], [np.nanmax(ob) if len(ob) > 0 else np.nan for ob in my_data[res]['max_ctz']], 'rx')
# Panel formatting
axb.set_yticklabels([''])
axb.set_xlabel('Time (hrs)')
axb.set_xticks(range(0, int(my_data[res][conv_time_key].max() + 1), [inc for inc in [6, 24] if my_data[res][conv_time_key].max()/inc > 1][0]))
axb.set_ylim([0, 4000])
axb.set_xlim([0, my_data[res][conv_time_key].max()])
axb.set_title('b) DX' + res[:-2] + 'S')
