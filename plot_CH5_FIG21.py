...
for key in keys:
    print key
    with Dataset(paths[key] + 'bouy_' + ['04' if key == 'U05' else '09'][0] + '.nc', 'r') as bouy_nc:
        time_key = [tkey for tkey in bouy_nc.variables.keys() if 'min' in tkey][0]
        times = bouy_nc.variables[time_key][:]*1. + [240.0 if key == 'U05' else 0.0][0]
...
plt.savefig('../Ch5_Figure21.png', dpi = 250, bbox_inches = 'tight')
plt.show()
