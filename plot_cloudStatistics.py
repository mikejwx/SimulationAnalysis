...
### Make some plots ###
my_colors = {'0200m' : 'gold', '0400m' : 'orange', '0800m' : 'red', '1600m' : 'darkred'}
# N clouds
fig = plt.figure()
...
for exp in experiments:
    ...
    ax.plot(np.array([my_data[exp]['profiles']['n_clds'][it,:] for it in range(my_it0, my_it1)]).mean(axis = 0), z/1000., color = my_colors[exp], label = 'DX' + exp[:-1])

ax.set_xlabel(u'$\hat{N}_{cld}$')
ax.set_ylabel(u'Height (m)')
ax.set_ylim([0, 3.5])
ax.set_xlim([0, 0.012])
ax.text(0.001, 3.25, 'a)')
...
ax.set_ylim([0, 3.5])
ax.set_xlim([0, 0.012])
ax.text(0.001, 3.25, 'b)')
ax.legend(loc = 0, frameon= = False)
plt.savefig('../cloudCountProfiles_dx.png', dpi = 150, bbox_inches = 'tight')
plt.show()
...
ax = fig.add_subplot(1, 1, 1)
for exp in experiments:
    my_it = np.where(my_data[exp]['mcl_times'] == 720.)[0][0]
...
for exp in experiments:
    ...
    ax.plot(median, z/1000., color = my_colors[exp], label = 'DX' + exp[:-1], lw = 2)
...
ax.set_ylim([0, 3.5])
ax.legend(loc = 0, frameon = False)
ax.set_title('11PM - 12AM')
ax.text(0.25, 3.25, 'b)')
plt.savefig('../cloudRadiiProfiles_dx.png', dpi = 150, bbox_inches = 'tight')
plt.show()
