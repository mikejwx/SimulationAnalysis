...
my_levels = [5., 10., 20., 50., 100., 200., 300., 400., 500., 600.]
...
def get_cm(it):
    ...
    #top left
    ax.contourf(X-np.max(X), Y+np.max(Y), lwp_data[it,:,:], colors = my_cmap(np.arange(9)/8.), levels = my_levels, extend = 'max')
    ax.contour(X-np.max(X), Y+np.max(Y), lsm_data[0,0,:,:], levels = [1e-08], colors = ['r'])
    #top centre
    ax.contourf(X, Y+np.max(Y), lwp_data[it,:,:], colors = my_cmap(np.arange(9)/8.), levels = my_levels, extend = 'max')
    ax.contour(X, Y+np.max(Y), lsm_data[0,0,:,:], levels = [1e-08], colors = ['r'])
    #top right
    ax.contourf(X+np.max(X), Y+np.max(Y), lwp_data[it,:,:], colors = my_cmap(np.arange(9)/8.), levels = my_levels, extend = 'max')
    ax.contour(X+np.max(X), Y+np.max(Y), lsm_data[0,0,:,:], levels = [1e-08], colors = ['r'])
    #centre left
    ax.contourf(X-np.max(X), Y, lwp_data[it,:,:], colors = my_cmap(np.arange(9)/8.), levels = my_levels, extend = 'max')
    ax.contour(X-np.max(X), Y, lsm_data[0,0,:,:], levels = [1e-08], colors = ['r'])
    #centre centre
    ax.contourf(X, Y, lwp_data[it,:,:], colors = my_cmap(np.arange(9)/8.), levels = my_levels, extend = 'max')
    ax.contour(X, Y, lsm_data[0,0,:,:], levels = [1e-08], colors = ['r'])
    #centre right
    ax.contourf(X+np.max(X), Y, lwp_data[it,:,:], colors = my_cmap(np.arange(9)/8.), levels = my_levels, extend = 'max')
    ax.contour(X+np.max(X), Y, lsm_data[0,0,:,:], levels = [1e-08], colors = ['r'])
    #bottom left
    ...
    ax.contour(X-np.max(X), Y-np.max(Y), lsm_data[0,0,:,:], levels = [1e-08], colors = ['r'])
    #bottom centre
    alb = ax.contourf(X, Y-np.max(Y), lwp_data[it,:,:], colors = my_cmap(np.arange(9)/8.), levels = my_levels, extend = 'max')
    axins = inset_axes(ax, width = "50%", height = "5%", loc = 8, bbox_to_anchor = (0.0, -0.26, 1, 1), bbox_transform = ax.transAxes, borderpad = 0.)
    plt.colorbar(alb, cax = axins, orientation = 'horizontal', label = r'LWP (g m$^{-2}$)')
    ax.contour(X, Y-np.max(Y), lsm_data[0,0,:,:], levels = [1e-08], colors = ['r'])
    #bottom right
    ax.contourf(X+np.max(X), Y-np.max(Y), lwp_data[it,:,:], colors = my_cmap(np.arange(9)/8.), levels = my_levels, extend = 'max')
    ax.contour(X+np.max(X), Y-np.max(Y), lsm_data[0,0,:,:], levels = [1e-08], colors = ['r'])
    ...
    ax.set_title('Liquid Water Path at T+' + "{0:04d}".format(int(times[it]))+' mins')
...
p = Pool()
p.map(get_cm, xrange(len(times)))
p.close()

command = 'convert -delay 30 -loop 0 ../cloudmasks/*.png ../cloudmasks/cm_anim.gif'
os.system(command)
