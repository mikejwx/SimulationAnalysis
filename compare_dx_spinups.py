...
### Read the data ###
exps  = ['100m', '200m', '400m', '800m', '1600m']
paths = {'100m' : '/nerc/n02/n02/xb899100/CloudTrail/Spinup_Control_0100m_HRIC_INV/',
...
plt.savefig('../SpinupDXComparison_metrics.png', dpi = 150)
plt.show()
...
for exp in exps:
    # Plot u-wind
    axa.plot(np.nanmean(my_data[exp][u_key], axis = 0), z_theta, color = exp_cols[exp], lw = 2, label = 'u-wind ' + exp)
    # Plot v-wind
    axa.plot(np.nanmean(my_data[exp][v_key], axis = 0), z_theta, color = exp_cols[exp], lw = 2, ls = '--', label = 'v-wind ' + exp)
    # Plot theta
    axb.plot(np.nanmean(my_data[exp][theta_key], axis = 0), z_theta, color = exp_cols[exp], lw = 2, label = u'$\\theta$ ' + exp)
    # Plot q
    axc.plot(np.nanmean(my_data[exp][q_key], axis = 0)*1000.0, z_theta, color = exp_cols[exp], lw = 2, ls = '--', label = u'q$_{v}$ ' + exp)

axa.plot([0,0], [0, 40000], lw = 0.5, ls = ':', color = 'grey')
axa.legend(loc = 'upper center')
axb.legend(loc = 'center left')
axc.legend(loc = 'right')
plt.savefig('../SpinupDXComparison_profiles.png', dpi = 150)
plt.show()
...
for exp in exps:
    axa.plot(my_data[exp][mcl_key].mean(axis = (0, 2, 3))*1000., z_theta/1000., color = exp_cols[exp], lw = 2, label = exp)
    axb.plot(np.where(my_data[exp][mcl_key] > 0, 1., 0.).mean(axis = (0, 2, 3))*100., z_theta/1000., color = exp_cols[exp], lw = 2, label = exp)

axa.legend(loc = 0)
axb.legend(loc = 0)
plt.savefig('../SpinupDXComparison_CloudCover-CloudLiquidProfiles.png', dpi = 150)
plt.show()
