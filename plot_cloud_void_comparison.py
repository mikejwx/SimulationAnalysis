...
labels = ['H050E250', 'H100E250', 'H150E250', 'H200E250', 'Control', 'H300E250', 'H350E250', 'H400E250']
...
for label in labels:
    ...
    axb.contour(x/1000., y/1000., cloud_frequency, colors = [my_cmap((labels.index(label)+1)/(len(labels)+1))], levels = [0.25])
    axb.contour(x/1000., y/1000., R, levels = [island_radius], colors = ['b'], linewidths = [2])
    axb.text((5./115.)*116, (0.425/0.5)*32, 'b)', bbox = dict(ec = 'None',fc = 'w', pad = 0.2))
    axb.set_ylabel('y (km)')
    axb.set_xlabel('x (km)')

plt.savefig('../cloud_void_cloud_frequency_comparison.png', dpi = 150, bbox_inches = 'tight')
plt.show()
